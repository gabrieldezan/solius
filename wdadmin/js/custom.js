...
$(document).ready(function () {
    ...
    $('#abre-modal_sair').click(function () {
        swal({
            title: "Você deseja realmente sair do administrador do site?",
            type: "warning",
...
function lista_paginas_menu() {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#lista_paginas").html("");
            $("#lista_paginas").append("<li><a href=\"" + vsUrl + "paginas-gerenciamento\">Gerenciamento</a></li>");
            if (data != 0) {
                for (i = 0; i < data.length; i++) {
                    $("#lista_paginas").append("<li onclick=\"set_id_pagina_session('" + data[i].id_paginas + "', '" + data[i].titulo + "')\"><a href=\"" + vsUrl + "informacoes\">" + data[i].titulo + "</a></li>");
                }
            }
        },
        error: function () {
...
function set_id_pagina_session(vsId, vsTitulo) {
    $.ajax({
        type: 'POST',
...
/*FUNÇÃO QUE BUSCA TODAS AS INFORMAÇÕES DA REUNIÃO*/
function logoff() {

    $.ajax({
        url: vsUrl + "controllers/Logoff.php",
        type: "POST",
        dataType: "json",
        success: function (data) {
            if (data == 1) {
                $(location).attr('href', vsUrl);
            } else {
                AvisoPersonalizado("Ocorreu um erro ao tentar fazer logoff no Intranet");
            }
        },
        error: function () {
            Erro();
