$(document).ready(function () {
    ...
    vsUrl = $("#vsUrl").val();
    viIdPaginas = $("#hiddenIdPaginas").val();
    ...
    monta_form(viIdPaginas);
    verifica_edicao();

});

/*FUNÇÃO QUE MONTA O FORMULÁRIO*/
function monta_form(viIdPaginas) {
    $.ajax({
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data !== 0) {
                ...
                $("#inputImagemAtual").val(data[0].imagem_destaque);
                $("#imgImagemAtual").attr("src", vsUrl + "uploads/informacoes/" + data[0].imagem_destaque);
