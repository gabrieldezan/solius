$(document).ready(function () {
    ...
    $(this).attr("title", "WD Admin - Serviços");

    $('#div_tabela').hide();
    $('#div_aviso').hide();
       
    $('#tabela_servicos').DataTable();
...
function carrega_servicos() {
    ...
    $.ajax({
        url: vsUrl + "controllers/RetornaServicos.php",
        type: "POST",
        dataType: "json",
        async: false,
        success: function (data) {
            if (data !== 0) {
                ...
                $('#tabela_servicos').DataTable().destroy();
                var table = $('#tabela_servicos').DataTable({
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            action: function () {
                                window.location.href = "servicos/cadastro";
                            }
                        }
                    ],
                    fixedHeader: false,
                    ...
                    "columns": [
                        ...
                        {
                            "render": function (data, type, row) {
                                return "<a href=\"servicos/cadastro/" + row.id_servicos + "\" class=\"btn btn-sm btn-outline-secondary\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Editar " + row.titulo + "\"><i class=\"fas fa-edit\"></i></a>&nbsp;";
                            }
                        }
                    ],
                    'columnDefs': [
